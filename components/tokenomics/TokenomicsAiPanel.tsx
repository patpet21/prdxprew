...
export const TokenomicsAiPanel: React.FC<TokenomicsAiPanelProps> = ({ 
  ...
  onCloseInsight,
  icon = "📊",
  color = "bg-blue-50 text-sim-ai border-blue-100"
}) => {
  return (
    <div className={`rounded-xl border border-dashed transition-all mb-6 ${insight ? 'bg-white border-brand-200' : 'bg-slate-50 border-sim-border'}`}>
      <div className="p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-start gap-4">
           <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shadow-sm shrink-0 bg-white border border-sim-border text-sim-ai`}>
             {icon}
           </div>
           <div>
        ...
        <Button 
          ...
          onClick={onAction} 
          isLoading={isLoading}
          className="bg-sim-cta hover:bg-sim-cta-hover text-white shadow-lg whitespace-nowrap"
        >
          {buttonText}
        </Button>
      </div>
