...
export const ExecutiveSummary: React.FC<Props> = ({ data }) => {
  ...
  // Use generated data or fall back to defaults
  const riskGrade = summary.aiRiskGrade || 'Pending'; 
  const valProp = summary.aiValueProposition || projectInfo.valueProposition || "Investment rationale pending generation...";
  const highlights = summary.keyHighlights || ["Asset Valuation", "Legal Structure", "Tokenomics Model"];
    ...
    <div className="animate-fadeIn space-y-8">
            ...
            <div className="relative z-10">
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                     {highlights.map((h: string, i: number) => (
                         <span key={i} className="px-3 py-1 bg-white/10 rounded-full text-xs font-bold text-indigo-200 border border-white/10">
                             ✓ {h}
                         </span>
                     ))}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-900 rounded-xl p-5 border border-slate-700 hover:border-amber-500/50 transition-colors group">
                ...
                <div className="text-xs text-slate-500 mt-1">
                    Min: ${(data.proDistribution?.minTicketSize || data.distribution.minInvestment || 0).toLocaleString()}
                </div>
            </div>
