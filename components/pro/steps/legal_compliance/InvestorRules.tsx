...
interface Props {
  data: any;
  updateData: (section: any, payload: any) => void;
}
...
export const InvestorRules: React.FC<Props> = ({ data, updateData }) => {
  ...
  const [inputs, setInputs] = useState({
      regulatoryRegime: compliance.primaryRegulationLane || 'Reg D 506(c)',
      totalInvestors: (compliance as any).totalInvestors || 50,
      retailCount: (compliance as any).retailCount || 0,
      accreditedCount: (compliance as any).accreditedCount || 45,
      institutionalCount: (compliance as any).institutionalCount || 5,
      includesUSInvestors: (compliance as any).includesUSInvestors !== false, // default true
      maxInvestmentPerInvestor: 0,
      minInvestmentTicket: compliance.minTicketSize || 5000,
  ...
  const saveToStorage = (validation = aiOutput, inEdu = inputEducation, outEdu = outputEducation) => {
      ...
      updateData('proCompliance', { ...compliance, ...inputs });
  };
