...
export const ProVisionStep: React.FC<StepProps & { activeTabId?: string, onTabChange?: (id: string) => void, onNextStep?: () => void }> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<VisionTab>(visibleTabs[0]?.id as VisionTab || 'project_identity');
  ...
  // Safety: Reset active tab if it disappears from visibleTabs (e.g. asset class change)
  useEffect(() => {
      if (!visibleTabs.find(t => t.id === activeTab)) {
          setActiveTab(visibleTabs[0]?.id as VisionTab || 'project_identity');
      }
  ...
  const currentTab = visibleTabs[currentIndex];
