...
import { calculateLTV } from '../../../../utils/financialUtils';
...
export const CapitalStack: React.FC<Props> = ({ data }) => {
  ...
  // Use robust LTV calculator
  const { ltv, riskLevel } = calculateLTV(seniorDebt + mezzDebt, valuation);
        ...
        <div className="flex items-center justify-between bg-slate-800 p-4 rounded-xl border border-slate-700">
            <div>
                ...
                <div className="flex items-baseline gap-2">
                    <span className={`text-2xl font-mono font-bold ${getRiskColor(riskLevel)}`}>
                        {ltv.toFixed(1)}%
                    </span>
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${riskLevel === 'Safe' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-500/30' : riskLevel === 'Moderate' ? 'bg-amber-900/30 text-amber-400 border-amber-500/30' : 'bg-red-900/30 text-red-400 border-red-500/30'}`}>
                        {riskLevel}
                    </span>
                </div>
            </div>
