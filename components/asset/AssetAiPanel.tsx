...
export const AssetAiPanel: React.FC<AssetAiPanelProps> = ({ 
  ...
  return (
    <div className={`rounded-xl border p-5 transition-all ${isAutofill ? 'bg-indigo-50 border-indigo-100' : 'bg-white border-sim-border shadow-sm'}`}>
      <div className="flex justify-between items-start gap-4">
        <div className="flex items-start gap-3">
           <div className={`p-2 rounded-lg text-lg ${isAutofill ? 'bg-white text-sim-ai shadow-sm' : 'bg-brand-50 text-sim-ai'}`}>
             {isAutofill ? '✨' : '🤖'}
           </div>
           <div>
        ...
        <Button 
          ...
          onClick={onAction} 
          isLoading={isLoading}
          className={isAutofill ? 'bg-sim-ai text-white hover:bg-purple-600' : 'bg-sim-cta text-white hover:bg-sim-cta-hover'}
        >
          {buttonText}
        </Button>
      </div>
