...
import { generateFullBusinessPlan } from '../services/mockAiService'; // Renamed to generateFullBusinessPlan in mockAiService for consistency
...
export const BusinessPlanGenerator: React.FC<BusinessPlanGeneratorProps> = ({ asset, projectInfo, onUpdate }) => {
  ...
  const handleGenerate = async () => {
    ...
    // Construct full state or context needed for generateFullBusinessPlan
    // For simplicity, we assume generateFullBusinessPlan can take partial or structured data
    // In a real app, you'd pass the full TokenizationState or reconstruct it.
    const mockData = { property: asset, projectInfo: projectInfo, jurisdiction: {}, compliance: {}, distribution: {}, tokenAllocation: {} }; // Mock full state
    const plan = await generateFullBusinessPlan(mockData);
    onUpdate(plan);
