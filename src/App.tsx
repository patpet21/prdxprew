...
import { UnlockAccessPage } from './pages/UnlockAccessPage';
...
type AppRoute = 
  ...
  // ACCESS ROUTES
  | 'ACCESS_REQUEST' | 'UNLOCK_ACCESS';

const STORAGE_KEY = 'pdx_simulator_state';
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-red-50 p-4 text-center">
          <div className="max-w-md bg-white p-8 rounded-xl shadow-xl border border-red-100">
            ...
            <p className="text-slate-600 mb-6 text-sm">Please reload the application. {this.state.error?.message}</p>
            <button onClick={() => window.location.reload()} className="px-6 py-2 bg-red-600 text-white rounded-lg">Reload</button>
          </div>
        </div>
      );
    }
    // Fix: cast to any to resolve property 'props' does not exist on type 'ErrorBoundary' error in some envs
...
function AppContent() {
  ...
  // State initialization with Persistence via storageUtils
  const [data, setData] = useState<TokenizationState>(() => {
    const loaded = storageUtils.load<TokenizationState | null>(STORAGE_KEY, null);
    if (loaded) {
        // Sanitization Fix: Ensure string fields are strings, not objects (fix for React Error #31)
        if (loaded.aiTokenomicsInsight && typeof loaded.aiTokenomicsInsight !== 'string') {
           loaded.aiTokenomicsInsight = undefined;
        }
        
        return {
          ...INITIAL_STATE,
          ...loaded,
          projectInfo: { ...INITIAL_STATE.projectInfo, ...loaded.projectInfo },
          jurisdiction: { ...INITIAL_STATE.jurisdiction, ...loaded.jurisdiction },
          property: { ...INITIAL_STATE.property, ...loaded.property },
          distribution: { ...INITIAL_STATE.distribution, ...loaded.distribution }
        };
    }
  ...
  // AUTO-SAVE EFFECT
  useEffect(() => {
    // Only save if we are in simulation mode (wizard) to avoid overwriting with empty state unintentionally
    if (route === 'SIM_WIZARD') {
      storageUtils.save(STORAGE_KEY, data);
    }
  }, [data, route]);
  ...
  const handleResetSimulation = () => {
    ...
    // Force a deep copy of initial state to ensure total reset
  ...
  // --- PRO LOGIC (ACCESS GATING) ---
  const handleNavigate = (page: string) => {
      // PRO_WELCOME, ENTERPRISE, and PRO_ACADEMY are accessible without paywall
      // PRO_ACADEMY is treated as the "Free Academy" per requirements
      if (page.startsWith('PRO_') && page !== 'PRO_WELCOME' && page !== 'PRO_ACADEMY') {
  ...
  const renderContent = () => {
    switch (route) {
        ...
        case 'SIM_WIZARD': return <WizardPage currentStep={currentStep} data={data} isStepValid={isStepValid} onNext={handleWizardNext} onBack={handleWizardBack} updateData={updateData} setIsStepValid={setIsStepValid} onCancel={() => setRoute('HOME')} onDeploy={handleDeployComplete} onReset={handleResetSimulation} isLoggedIn={!!userProfile} onLogin={handleLogin} />;
        ...
        case 'DASHBOARD': return <DashboardPage projects={projects} onCreateNew={handleDashboardCreateNew} onLogout={handleLogout} onGoHome={() => setRoute('HOME')} onGoToTrading={() => setRoute('TRADING')} onNavigate={handleNavigate} />;
        
        // ACCESS GATING
        case 'ACCESS_REQUEST': return <AccessRequestPage onNavigate={handleNavigate} onBack={() => setRoute('HOME')} />;
        case 'UNLOCK_ACCESS': return <UnlockAccessPage onUnlock={() => handleUpgradeToPro()} onNavigate={handleNavigate} />;
